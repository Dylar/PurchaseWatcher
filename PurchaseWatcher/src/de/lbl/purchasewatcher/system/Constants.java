package de.lbl.purchasewatcher.system;

public class Constants
{
	public static final int ACTION_DISPLAY_VIEW = 0;
	
	public static final String KEY_ID = "key_id";
	
	public static final String KEY_VIEW_NAME = "key_view_name";
	public static final String KEY_VIEW_TITLE = "key_view_title";
	public static final String KEY_VIEW_MODE = "key_view_mode";
	
	public static final String KEY_DATA_THINGYS = "key_data_thingys";
	public static final String KEY_DATA_THINGY = "key_data_thingy";
	public static final String KEY_DATA_PURCHASE = "key_data_purchase";
	
	
	public static final String VIEW_PURCHASE = "view_purchase";
	public static final String VIEW_THINGY = "view_thingy";
	public static final String VIEW_CALENDAR = "view_calendar";
	public static final String VIEW_EXPENDITURE = "view_expenditure";
	public static final String VIEW_START = "view_start";
	
	public static final String MODE_OVERVIEW = "mode_overview";
	public static final String MODE_EDIT = "mode_edit";
	public static final String MODE_DETAIL = "mode_detail";

	
	//public static final int SHOW_VIEW_THINGY_EDITOR = 1;
	//public static final int SHOW_VIEW_PURCHASE_OVERVIEW = 2;
}
